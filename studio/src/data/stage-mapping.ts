...
// =============================================================================
// review 报告定位（next.md 第 24 行 + 第 75 行规则）
// 审查报告默认与被审产物同目录，文件名 review-{stage_token}-{spec_id}.md
// 两个例外：swe_coding_plan 在 coding-plan/，swe_coding_do 的 token 是 swe_coding
// =============================================================================
export function getReviewDir(stage: StageToken, projectSpecId: string): string[] {
  const m = STAGE_TABLE[stage];
  // 例外①：swe_coding_plan 审查报告在 project/tasks/coding-plan/
  if (stage === 'swe_coding_plan') {
    return ['project/tasks/coding-plan'];
  }
  // 例外②：swe_coding_do 源码符合性审查 token=swe_coding，报告在 project/tasks/coding-do/
  if (stage === 'swe_coding_do') {
    return ['project/tasks/coding-do'];
  }
  // 默认：从 spec_globs 第一项提取产物目录
  if (m.spec_globs.length === 0) return [];
  const firstGlob = m.spec_globs[0];
  // 取到 .md 之前的目录
  const dir = firstGlob.split('*')[0].replace(/\/$/, '');
  return [dir];
}

export function getReviewFileGlob(stage: StageToken, projectSpecId: string): string[] {
  const dirs = getReviewDir(stage, projectSpecId);
  const reviewToken = stage === 'swe_coding_do' ? 'swe_coding' : stage;
  return dirs.map((d) => `${d}/review-${reviewToken}-${projectSpecId}.md`);
}

export function getSignoffFileGlob(stage: StageToken, projectSpecId: string): string[] {
  return getReviewFileGlob(stage, projectSpecId).map((f) => f.replace(/\.md$/, '-signoff.md'));
}

// =============================================================================
// 任务文件定位（处理别名）
// =============================================================================
const TASK_FILE_ALIASES: Record<string, string[]> = {
  // stage_token -> 备选文件名（按优先顺序）
  swe_analysis: ['task_sw_req.md', 'task_sw_req_from_prd.md'],
  swe_arch_if: ['task_sw_arch_if.md'],
  swe_unit_verify: ['task_sw_ut.md'],
};

export function getTaskFileCandidates(stage: StageToken): string[] {
  const m = STAGE_TABLE[stage];
  if (!m.task_file) return [];
  if (m.task_file.endsWith('/')) {
    // 目录（如 coding-plan/）
    return [`project/tasks/${m.task_file}task_*.md`];
  }
  const base = `project/tasks/${m.task_file}`;
  // 处理别名
  const aliases = TASK_FILE_ALIASES[stage] || [m.task_file];
  return aliases.map((a) => `project/tasks/${a}`);
}
